...
pi_list  = json.load(open('data/gpt2_estimated_prior.json'))
...
for exp_name in exp_names:
    ...
    #fit.save_csvfiles(dir="stan_model/mcmc_output/gpt2_norm/{}".format(exp_name))

    start_time = time.time()
    model_fit = arviz.from_cmdstanpy(fit)
    print('Converting Stan fit to inference object takes', (time.time() - start_time), 's')

    savepath = 'stan_model/model_fit/gpt2_norm/{exp_name}_{model_name}_model_fit.nc'.format(exp_name=exp_name, model_name=model_name)
