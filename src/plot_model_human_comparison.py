...
def plot_model_human_comparison_across_two_studies(model_fit_all, observed_freqs, model_names, pretty_model_names, savepath=None):
    n_cond = 4
    n_e_type = 4
    markersize = 5
    alpha = 0.8
        ...
        for model_idx, model_name in enumerate(model_names):
            ...
            for i, index in enumerate(items):
                for j in range(n_cond):
                    observed_e_freq = normalize(observed_all[i, j])
...
# Load observed frequency count of edit/error types as well as model fits
observed_freqs = {}
model_fit_all = {}
...
plot_model_human_comparison_across_two_studies(model_fit_all, observed_freqs, model_names, pretty_model_names, savepath=savepath)
