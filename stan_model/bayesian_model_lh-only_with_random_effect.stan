...
transformed parameters {
  ...
  for (n in 1:N){
    for (k in 1:K){
      if (k == 1){
        log_inferred_e_prob[n, k, 1] = log(error_prob[3, 1]);
        log_inferred_e_prob[n, k, 2] = log(error_prob[1, 2]);
        log_inferred_e_prob[n, k, 3] = log(error_prob[4, 3]);
        log_inferred_e_prob[n, k, 4] = log(error_prob[2, 4]);
      }else if (k == 2){
        log_inferred_e_prob[n, k, 1] = log(error_prob[4, 1]);
        log_inferred_e_prob[n, k, 2] = log(error_prob[2, 2]);
        log_inferred_e_prob[n, k, 3] = log(error_prob[3, 3]);
        log_inferred_e_prob[n, k, 4] = log(error_prob[1, 4]);
      }else if (k == 3){
        log_inferred_e_prob[n, k, 1] = log(error_prob[1, 1]);
        log_inferred_e_prob[n, k, 2] = log(error_prob[3, 2]);
        log_inferred_e_prob[n, k, 3] = log(error_prob[2, 3]);
        log_inferred_e_prob[n, k, 4] = log(error_prob[4, 4]);
      }else {
        log_inferred_e_prob[n, k, 1] = log(error_prob[2, 1]);
        log_inferred_e_prob[n, k, 2] = log(error_prob[4, 2]);
        log_inferred_e_prob[n, k, 3] = log(error_prob[1, 3]);
        log_inferred_e_prob[n, k, 4] = log(error_prob[3, 4]);
      }
    }
  }
