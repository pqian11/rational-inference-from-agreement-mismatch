...
transformed parameters {
  ...
  for (n in 1:N){
    for (k in 1:K){
      if (k == 1){
        log_inferred_e_prob[n, k, 1] = log(prior[n, 3])*a;
        log_inferred_e_prob[n, k, 2] = log(prior[n, 1])*a;
        log_inferred_e_prob[n, k, 3] = log(prior[n, 4])*a;
        log_inferred_e_prob[n, k, 4] = log(prior[n, 2])*a;
      }else if (k == 2){
        log_inferred_e_prob[n, k, 1] = log(prior[n, 4])*a;
        log_inferred_e_prob[n, k, 2] = log(prior[n, 2])*a;
        log_inferred_e_prob[n, k, 3] = log(prior[n, 3])*a;
        log_inferred_e_prob[n, k, 4] = log(prior[n, 1])*a;
      }else if (k == 3){
        log_inferred_e_prob[n, k, 1] = log(prior[n, 1])*a;
        log_inferred_e_prob[n, k, 2] = log(prior[n, 3])*a;
        log_inferred_e_prob[n, k, 3] = log(prior[n, 2])*a;
        log_inferred_e_prob[n, k, 4] = log(prior[n, 4])*a;
      }else {
        log_inferred_e_prob[n, k, 1] = log(prior[n, 2])*a;
        log_inferred_e_prob[n, k, 2] = log(prior[n, 4])*a;
        log_inferred_e_prob[n, k, 3] = log(prior[n, 1])*a;
        log_inferred_e_prob[n, k, 4] = log(prior[n, 3])*a;
      }
    }
  }
